...
import com.kang.task.thread.TopnewsThread;
...
public class TopnewsScheduleTask  implements ScheduleTask {
    ...
    public TopnewsScheduleTask(@Qualifier("topnewsThreadPoolTaskScheduler") @Lazy ThreadPoolTaskScheduler botThreadPoolTaskScheduler) {
        this.botThreadPoolTaskScheduler = botThreadPoolTaskScheduler;
    }

    @Bean
    public ThreadPoolTaskScheduler topnewsThreadPoolTaskScheduler() {
     ...
     */
    @Override
    @GetMapping("/topnewsScheduleTask/restartCron")
    public int onOrReleaseTask() {
        log.info("\n开始或重启定时任务：每日新闻");
        //从数据库查询对应的定时任务
        QueryWrapper<ScheduleTime> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("schedule_name", "topnewsScheduleTask");
        ...
        future = botThreadPoolTaskScheduler.schedule(new TopnewsThread(), new CronTrigger(format));
     ...
     */
    @Override
    @GetMapping("/topnewsScheduleTask/stopCron")
